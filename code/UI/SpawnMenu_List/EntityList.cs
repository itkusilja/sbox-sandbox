...
public partial class EntityList : Panel
{
	...
	public bool IsEnt(Sandbox.LibraryAttribute x)
	{
		if (!x.Spawnable) return false;
		if (!x.Name.StartsWith("ent_")) return false;
		return true;
	}
	...
	public EntityList()
	{
		...
		var ents = Library.GetAllAttributes<Entity>().Where( x => IsEnt(x)).OrderBy( x => x.Title ).ToArray();
